...
def generate_xlsx_document(data_frames,sheets,config,user_columns,package_type, WTD_mode,tww,fabNU,config_str):
    overall_paretos = {}
    for conf in config:
        overall_paretos.update( {conf : pd.DataFrame()} ) # Update Overall Pareto with configs
    yield_dic = {}
    output_workbook_name2 = output_workbook_name.split(".xlsx")[0]+"_"+str(config_str)+"_"+str(tww)+"_"+str(fabNU)+".xlsx" #rename file name with config,tww,fab
...
by config and STEP for REGISTER             example: group name$x16*BURN^comments^owner^ETA will only output to x16 BURN Registers.')
    ...
    generate_xlsx_document(data_frames,sheets,config,user_columns,package_type, WTD_mode,tww,fabNU,config_str)
