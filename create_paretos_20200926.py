...
sheets = []
file_directory = 'csv_files_20200926/' # directory for supporting files such as csv data files
...
def generate_xlsx_document(data_frames,sheets,config,user_columns,package_type, WTD_mode):
    overall_paretos = {}
    for conf in config:
...
by config and STEP for REGISTER             example: group name$x16*BURN^comments^owner^ETA will only output to x16 BURN Registers.')
    ...
    generate_xlsx_document(data_frames,sheets,config,user_columns,package_type, WTD_mode)
